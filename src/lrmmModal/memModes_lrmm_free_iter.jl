...
function run_case( mfac, tfac, rS )
  ...
  name::String = "data/sims_202508/mem_modes_free/mem_modes_"*caseName
  order::Int = 2
  vtk_output::Bool = true
  filename = name*"/mem"

  if( isdir(name) )
    rm(name, recursive=true) #remove old data
    @show "Removed old data in ", name
    # return
  end
  mkdir(name)
  ...
  c14(q,v) = -rS.K*δ_p( v*(q⋅î1) ) 
  c41(η,ξ) = -rS.K*δ_p((ξ⋅î1)*η)

  m44(q,ξ) = rS.M*δ_p(q⋅ξ)
  k44(q,ξ) = rS.K*δ_p(q⋅ξ)
