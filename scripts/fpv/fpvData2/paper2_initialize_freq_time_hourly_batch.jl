...
setList = CSV.read(scriptsdir("fpv","fpvData2",
  "hourly_All.csv"), DataFrame)

fillRatio = 100

## Start processing
daHs = CSV.read(
  "data/fpv_202409/dataHsTmHourly/sign_wave_height.csv",
  DataFrame )

daTz = CSV.read(
  "data/fpv_202409/dataHsTmHourly/average_period.csv",
  DataFrame )

daDay = CSV.read(
  "data/fpv_202409/dataHsTmHourly/daylight_flag.csv",
  DataFrame )
...
fileName = datadir(setList[testID,2], setList[testID,3])*"/"
mkpath(fileName)
...
daReportNames = [
  ...
  "depth", "Hs", "Tz", "Tp",
  "phaseSeed"]
daReport = DataFrame( zeros(nHours, length(daReportNames)), 
  daReportNames ) 

daReport[!,"Material"] .= "" #To convert this entry to string type
...
for i in axes(daHs,1)
  ...
  spectrum_dict = Dict(
      ...
      "phaseshift" => randomPhase(ω; seed = phaseRandomSeed)
  )
  ...
  daReport[i,:] = [da["material"], da["h_b"], fillRatio,
    i, day, 
    d, Hs, Tz, Tp, phaseRandomSeed]

  CSV.write(fileName*"aRunLog.csv", 
    daReport; delim = ';')
